...
def qphase(image, L1, L2, phi, ch, cv, sh, sv):
    ...
    a11 = np.multiply(a, a) + np.multiply(b, b) - np.multiply(c, c) - np.multiply(d, d)
    a21 = 2 * (np.multiply(b, c) + np.multiply(a, d))
    a31 = 2 * (np.multiply(b, d) - np.multiply(a, c))
    a12 = 2 * (np.multiply(b, c) - np.multiply(a, d))
    a22 = np.multiply(a, a) - np.multiply(b, b) + np.multiply(c, c) - np.multiply(d, d)
    a32 = 2 * (np.multiply(c, d) + np.multiply(a, b))
    a13 = 2 * (np.multiply(b, d) + np.multiply(a, c))
